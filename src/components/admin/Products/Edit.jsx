...
function Edit({ onSubmitSuccess, record, onClose }) {
  ...
  useEffect(() => {
    ...
    if (record?.product_category_id && categories.length > 0) {
      const categoryOption = categories.find(option => option.value === record.product_category_id);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axiosClient.post("products/update", formData);
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{t("Update Product")}</DialogTitle>
        </DialogHeader>
